...
OpenAgents はハッカソン参戦プロジェクトのため、**実装前に必ず `prizes` スキルで賞金トラックとの整合を確認する**。

1. `prizes` スキルを load し、`docs/prizes/*.md` の最新賞要件を反映する。
2. これから作る機能を `prizes` スキルのルーブリックで採点する (0G / ENS / Gensyn AXL / KeeperHub / Uniswap)。
3. 期待賞金が低い / 単一賞しか狙えない / cosmetic 統合になっている場合は、ヒアリング前にユーザーと「軸の付け替え」を相談する。
4. 採点結果は次フェーズで仕様書の **Prize Targets** セクションに転記する。
