...
type Mode = 'blueprint' | 'silhouette' | 'hot';
...
function FighterImpl({
  width = 880,
  mode = 'blueprint',
}: { width?: number; mode?: Mode }) {
  const stroke = mode === 'hot' ? PALETTE.amber : PALETTE.hud;
  const fill = mode === 'silhouette' ? '#0c1622' : 'none';
  const accent = mode === 'hot' ? PALETTE.amber : PALETTE.hud;
  const dim = mode === 'hot' ? '#7a5520' : PALETTE.hudDim;
  ...
  return (
    ...
    >
      ...
      <g stroke={stroke} fill={fill} strokeWidth="1.4" strokeLinejoin="miter">
        ...
        <path
          d="M 600 195 L 705 165 L 760 175 L 720 220 Z"
          fill={mode === 'silhouette' ? '#0a1420' : '#0d1825'}
        />
        <path
          d="M 600 365 L 705 395 L 760 385 L 720 340 Z"
          fill={mode === 'silhouette' ? '#0a1420' : '#0d1825'}
        />
        ...
        <path d="M 760 273 L 760 287" stroke={dim} strokeWidth="0.8" />
        <path d="M 740 271 L 740 289" stroke={dim} strokeWidth="0.6" />
        ...
        <path
          ...
          opacity="0.7"
        />
        <path d="M 580 280 L 700 280" stroke={dim} strokeWidth="0.6" />
        ...
        <g>
          ...
          <ellipse
            ...
            rx="7"
            ry="4"
            fill={mode === 'silhouette' ? '#ff7a1a' : '#ff9a3d'}
          />
          <ellipse
            cx="200"
            cy="308"
        ...
        <path d="M 230 280 L 760 280" stroke={dim} strokeWidth="0.7" />

        <g stroke={dim} strokeWidth="0.6" fill="none" opacity="0.85">
        ...
        <g transform="translate(560 230)">
          <circle r="9" fill="none" stroke={dim} strokeWidth="1" />
